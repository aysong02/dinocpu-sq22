...
class PipelinedCPUBP(implicit val conf: CPUConfig) extends BaseCPU {
  ...
  nextPCmod.io.eqf    := alu.io.eqf
  nextPCmod.io.ltf    := alu.io.ltf
  nextPCmod.io.ltuf   := alu.io.ltuf
  nextPCmod.io.funct3 := id_ex.io.data.instruction(14,12)
  nextPCmod.io.pc_or_x:= Mux(id_ex_ctrl.io.data.ex_ctrl.jalr, forward_a_mux, id_ex.io.data.pc)
